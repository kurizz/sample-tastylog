...
passport.use(
  ...
  new Strategy({
    ...
    try {
      ...
      if (
        results[0].locked &&
        moment(now).isSameOrBefore(moment(results[0].locked).add(appconfig.security.ACCOUNT_LOCK_TIME, "minutes"))
      ) {
      ...
      if (!await bcrypt.compare(password, results[0].password)) {
        ...
        if (failureCount >= appconfig.security.ACCOUNT_LOCK_THRESHOLD) {
          await MySqlClient.executeQuery("UPDATE t_user SET locked = ? WHERE id = ?", [now, user.id])
        }
        return done(null, false, req.flash("message", "ユーザ名またはパスワードが間違っています。"));
      }
      ...
      await MySqlClient.executeQuery("UPDATE t_user SET locked = ? WHERE id = ?", [null, user.id])
